...
LOG_PROP *COMP_OP::FindLogProp(LOG_PROP **input) {
  ...
  if (op_code != OP_NOT) fv.Merge(RightProp->FreeVars);
  ...
  //  use magic number
  if (LeftProp->CuCard == -1 || RightProp->CuCard == -1 || (LeftProp->CuCard != 1 && RightProp->CuCard != 1) ||
      op_code == OP_LIKE) {
    // magic numbers
    switch (op_code) {
      case OP_LIKE:
    ...
    switch (op_code) {
      ...
      case OP_LE:
      case OP_LT:
        selectivity = (MIN(attr_max, const_value) - MIN(attr_min, const_value)) / (attr_max - attr_min);
        break;
      case OP_GE:
      case OP_GT:
        selectivity = (MAX(attr_max, const_value) - MAX(attr_min, const_value)) / (attr_max - attr_min);
        break;
    };
  }
  ...
  switch (op_code) {
    case OP_AND:
      ...
      break;
    case OP_OR:
      selectivity = LeftProp->Selectivity + RightProp->Selectivity - LeftProp->Selectivity * RightProp->Selectivity;
      break;
    case OP_NOT:
