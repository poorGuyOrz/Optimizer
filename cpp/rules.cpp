...
Expression *BINDERY::extract_expr() {
  ...
  else  // general invocation of new Expression
  {
    ...
    if (arity) {
      ...
      for (int input_no = 0; input_no < arity; input_no++) subexpr.push_back(input[input_no]->extract_expr());

      // Put everything together for the result.
      result = new Expression(op_arg, std::move(subexpr));
    } else
...
EQ_TO_LOOPS::EQ_TO_LOOPS()
    : Rule("EQJOIN->LOOPS_JOIN", 2,
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})}),
           new Expression(new LOOPS_JOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})})) {
...
Expression *EQ_TO_LOOPS::next_substitute(Expression *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  Expression *result = new Expression(new LOOPS_JOIN(lattrs, rattrs, size),
                                      {new Expression(*(before->GetInput(0))), new Expression(*(before->GetInput(1)))});
  return result;
}
...
EQ_TO_LOOPS_INDEX::EQ_TO_LOOPS_INDEX()
    : Rule("EQJOIN -> LOOPS_INDEX_JOIN", 1,
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new GET(0), {})}),
...
EQ_TO_MERGE::EQ_TO_MERGE()
    : Rule("EQJOIN -> MERGE_JOIN", 2,
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})}),
           new Expression(new MERGE_JOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})})) {
...
Expression *EQ_TO_MERGE::next_substitute(Expression *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  result = new Expression(new MERGE_JOIN(lattrs, rattrs, size),
                          {new Expression(*(before->GetInput(0))), new Expression(*(before->GetInput(1)))});
...
EQ_TO_HASH::EQ_TO_HASH()
    : Rule("EQJOIN->HASH_JOIN", 2,
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})}),
           new Expression(new HASH_JOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})})) {
  // set rule index
...
Expression *EQ_TO_HASH::next_substitute(Expression *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  result = new Expression(new HASH_JOIN(lattrs, rattrs, size),
                          {new Expression(*(before->GetInput(0))), new Expression(*(before->GetInput(1)))});
...
EQJOIN_COMMUTE::EQJOIN_COMMUTE()
    : Rule("EQJOIN_COMMUTE", 2,
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})}),
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(1), {}), new Expression(new LeafOperator(0), {})})) {
...
EQJOIN_LTOR::EQJOIN_LTOR()
    : Rule("EQJOIN_LTOR", 3,
           new Expression(
               new EQJOIN(0, 0, 0), {new Expression(new EQJOIN(0, 0, 0), {new Expression(new LeafOperator(0), {}),  // A
           ...
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}),
                           new Expression(new EQJOIN(0, 0, 0), {new Expression(new LeafOperator(1), {}),
                                                                new Expression(new LeafOperator(2), {})})}))
...
Expression *EQJOIN_LTOR::next_substitute(Expression *before, PHYS_PROP *ReqdProp) {
  ...
  Expression *result = new Expression(
      new EQJOIN(nlattrs2, nrattrs2, nsize2),
      {new Expression(*(before->GetInput(0)->GetInput(0))),
       new Expression(new EQJOIN(nlattrs1, nrattrs1, nsize1),
                      {new Expression(*(before->GetInput(0)->GetInput(1))), new Expression(*(before->GetInput(1)))})});
  return result;
    ...
    : Rule("EQJOIN_RTOL", 3,
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new LeafOperator(0), {}),                                       // A
                           new Expression(new EQJOIN(0, 0, 0), {new Expression(new LeafOperator(1), {}),  // B
                                                                new Expression(new LeafOperator(2), {})}  // C
                                          )}),  // original pattern
           new Expression(
               new EQJOIN(0, 0, 0), {new Expression(new EQJOIN(0, 0, 0), {new Expression(new LeafOperator(0), {}),  // A
...
Expression *EQJOIN_RTOL::next_substitute(Expression *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  Expression *result = new Expression(
      new EQJOIN(nlattrs2, nrattrs2, nsize2),  // args reversed
      {new Expression(new EQJOIN(nlattrs1, nrattrs1, nsize1),
                      {new Expression(*(before->GetInput(0))), new Expression(*(before->GetInput(1)->GetInput(0)))}),
...
EXCHANGE::EXCHANGE()
    : Rule("EXCHANGE", 4,
           new Expression(new EQJOIN(0, 0, 0),
                          {new Expression(new EQJOIN(0, 0, 0), {new Expression(new LeafOperator(0), {}),      // A
                                                                new Expression(new LeafOperator(1), {})}),    // B
                           new Expression(new EQJOIN(0, 0, 0), {new Expression(new LeafOperator(2), {}),      // C
...
//	a || !b
//
bool EXCHANGE::condition(Expression *before, MExression *mexpr, int ContextID) { return true; }  // EXCHANGE::condition
...
RM_TO_HASH_DUPLICATES::RM_TO_HASH_DUPLICATES()
    : Rule("RM_DUPLICATES  -> HASH_DUPLICATES", 1,
           new Expression(new RM_DUPLICATES(), {new Expression(new LeafOperator(0), {})}  // input table
                          ),
...
AL_TO_HGL::AL_TO_HGL(AGG_OP_ARRAY *list1, AGG_OP_ARRAY *list2)
    : Rule("AGG_LIST  -> HGROUP_LIST", 1,
           new Expression(new AGG_LIST(0, 0, list1), {new Expression(new LeafOperator(0), {})}  // input table
                          ),
...
FO_TO_PFO::FO_TO_PFO()
    : Rule("FUNC_OP  -> P_FUNC_OP", 1,
           new Expression(new FUNC_OP("", 0, 0), {new Expression(new LeafOperator(0), {})}  // input table
                          ),
...
Expression *AGG_THRU_EQJOIN::next_substitute(Expression *before, PHYS_PROP *ReqdProp) {
  ...
  delete[] gby_atts;
  // create transformed expression
  result = new Expression(new EQJOIN(lattrs, rattrs, size),
                          {new Expression(*(before->GetInput(0)->GetInput(0))),
...
EQ_TO_BIT::EQ_TO_BIT()
    : Rule("EQJOIN -> BIT_JOIN", 2,
           ...
           new Expression(new BIT_JOIN(0, 0, 0, 0),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})})) {
  // set rule index
...
Expression *EQ_TO_BIT::next_substitute(Expression *before, PHYS_PROP *ReqdProp) {
  ...
  result =
      new Expression(new BIT_JOIN(lattrs, rattrs, size, GetOp->GetCollection()),
...
PROJECT_THRU_SELECT::PROJECT_THRU_SELECT()
    : Rule("PROJECT_THRU_SELECT", 2,
           ...
           new Expression(new PROJECT(0, 0),
                          {new Expression(new SELECT,
                                          {new Expression(new PROJECT(0, 0), {new Expression(new LeafOperator(0), {})}),
                                           new Expression(new LeafOperator(1), {})})})) {
...
DUMMY_TO_PDUMMY::DUMMY_TO_PDUMMY()
    : Rule("DUMMY->PDUMMY", 2,
           new Expression(new DUMMY(),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})}),
           new Expression(new PDUMMY(),
                          {new Expression(new LeafOperator(0), {}), new Expression(new LeafOperator(1), {})})) {
